...
function flatClassName(name, prefix) {
  if (!name || !name.trim()) {
    return '';
  }
  return [].concat(name).join(' ').split(' ').map(function (item) {
    ...
    return function (_Component) {
      ...
      _createClass(_class, [{
        ...
        value: function render() {
          ...
          var cn = [].concat(className || [], typeof name === 'function' ? name(this.props) : name);
          var p = _fsts.JS.lessProps(this.props, 'className');

          return _react2.default.createElement(
            Comp,
            _extends({}, p, { className: cn.join(' ') }),
            this.props.children
          );
        }
      }]);
